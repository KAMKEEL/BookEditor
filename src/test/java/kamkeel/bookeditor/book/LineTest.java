...
public class LineTest extends AbstractBookTest {
    ...
    @Test
    public void wrappingPrefersWhitespaceBoundary() {
        String text = "Hello my name is GIANT WORLD";

        fontRenderer.setWidth(' ', 3);
        fontRenderer.setWidth('G', 4);
        fontRenderer.setWidth('I', 4);
        fontRenderer.setWidth('A', 4);
        fontRenderer.setWidth('N', 4);
        fontRenderer.setWidth('T', 4);

        List<String> wrapped = Line.listFormattedStringToWidth(text, "");

        assertThat("Expected at least two wrapped segments", wrapped.size(), greaterThan(1));
        assertThat("First segment should contain the preceding word", wrapped.get(0).contains("GIANT"), is(true));
        assertThat("First segment should not contain trailing word", wrapped.get(0).contains("WORLD"), is(false));
        assertThat("Overflow should keep the full word", wrapped.get(1), startsWith("WORLD"));
    }
